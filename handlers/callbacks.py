...
async def handle_callback(query: CallbackQuery, state: FSMContext):
    ...
    if query.data == "back_to_start":
        await state.clear()
        await query.message.edit_text("📁 Оберіть тип документа (крок 1 з 3):", reply_markup=get_main_type_keyboard())
        return 
    ...
    if data[0] == "authority":
        ...
        keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"doc_type:{doc_type}")])
    ...
    if data[0] == "subtype":
        ...
        if doc_type == "заява":
            ...
            # Рапорт — с ролями
            docx_id = doc.get("docx_commander") if authority == "командир" else doc.get("docx_chief")
            pdf_id = doc.get("pdf_commander") if authority == "командир" else doc.get("pdf_chief")

        if not docx_id or not pdf_id:
            await query.message.answer("⚠️ Для цього підтипу не знайдені файли. Зверніться до адміністратора.")
            return
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="📥 Завантажити", callback_data=f"download:{subtype_key}"),
    ...
    if data[0] == "sed_intruction":
        keyboard = InlineKeyboardMarkup(inline_keyboard=[    
            [InlineKeyboardButton(text="🔙 Назад", callback_data="sed")]
        ])
        await query.message.edit_text(
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="📥 Завантажити", callback_data=f"download_sed:{pdf_id}"),
                InlineKeyboardButton(text="👁️ Переглянути", url=pdf_url)
            ],
